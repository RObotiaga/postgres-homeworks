...
SELECT DISTINCT * FROM orders WHERE ship_city LIKE '%burg'
...
SELECT first_name, last_name, home_phone, region FROM employees WHERE region IS NOT NULL
...
SELECT SUM(freight), ship_country FROM orders WHERE ship_region IS NULL GROUP BY ship_country HAVING SUM(freight) > 2750 ORDER BY SUM(freight) DESC;

-- 6. страны, в которых зарегистрированы и заказчики (customers) и поставщики (suppliers) и работники (employees).
SELECT DISTINCT country FROM customers INTERSECT SELECT DISTINCT country FROM suppliers INTERSECT SELECT DISTINCT country FROM employees
...
SELECT DISTINCT country FROM customers INTERSECT SELECT DISTINCT country FROM suppliers EXCEPT SELECT DISTINCT country FROM employees